...
class BudgetApiTests(APITestCase):
    ...
    def test_budget_create_normalizes_month_and_returns_spending_metrics(self):
        ...
        response = self.client.post(
            "/api/budgets/",
            {"month": "2026-05-18", "limit_amount": "1000.00"},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.data["month"], "2026-05-01")
        self.assertEqual(response.data["spent_amount"], Decimal("250.00"))
        self.assertEqual(response.data["status"], "healthy")

    def test_budget_rejects_duplicate_month(self):
        self.client.post("/api/budgets/", {"month": "2026-05-01", "limit_amount": "1000.00"}, format="json")
        response = self.client.post(
            "/api/budgets/",
            {"month": "2026-05-21", "limit_amount": "2000.00"},
            format="json",
        )
