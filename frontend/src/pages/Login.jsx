...
.divider {
  ...
  margin: 18px 0 14px;
  opacity: 0;
  animation: riseIn 0.6s 0.94s cubic-bezier(0.22, 1, 0.36, 1) forwards;
}
...
.social-btn {
  ...
  transition: all 0.2s;
  opacity: 0;
  animation: riseIn 0.6s 0.97s cubic-bezier(0.22, 1, 0.36, 1) forwards;
}

.social-btn:hover:not(:disabled) { background: var(--surface-hover); border-color: var(--surface-border-3); }
...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      setSubmitError(error.message || "Unable to continue with Google right now.");
    } finally {
    ...
    <>
      ...
      <div className="shell">
        <div className="glass-card">
          ...
          <button className="social-btn" type="button" disabled={loading || googleLoading} onClick={handleGoogleSignIn}>
